...
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`country`) FROM `dsmtps` WHERE `sold` = '0' ORDER BY country ASC");
...
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`resseller`) FROM `dsmtps` WHERE `sold` = '0' ORDER BY resseller ASC");
	while($row = mysqli_fetch_assoc($query)){
...
<thead>
 ...
 <?php
						   include("cr.php");
     $q = mysqli_query($dbcon, "SELECT * FROM dsmtps WHERE sold='0' ORDER BY RAND()")or die(mysql_error());
