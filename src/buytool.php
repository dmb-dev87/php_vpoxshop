...
if($balance >= $price){
  ...
  if($ree['sold'] == '0') {
    ...
    $qyery = mysqli_query($dbcon, "INSERT INTO purchases (id,buyer,date,type,country,infos,url,login,pass,price,resseller,reported,reportid) VALUES ('$sid','$usrid','$date','$type','$fb','$infos','$url','$login','$pa','$price','$resseller','0','0') ") or die(mysqli_error($dbcon));
    $last_id = mysqli_insert_id($dbcon);
    $b = $price;
    mysqli_query($dbcon, "UPDATE resseller SET allsales=(allsales + $b),soldb=(soldb + $b) WHERE username='$resseller'");
    echo '<button onclick="openitem('.$sid.')" class="btn btn-primary btn-xs"> Order #'.$sid.'</button>';
  } else {
