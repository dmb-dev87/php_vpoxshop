    ...
    if($amount>= $row['amount']) {
      if ($row['state'] == "paid") {
        $y = $row['amountusd'];
		    ...
		    echo '{"stop":"1","time":"'.date("Y/m/d h:i:s").'","btc":"'.$amount.'","state":"Paid","error":"1","errorTXT":"'.$msg.'"}';
        echo '{"stop":"1","time":"'.date("Y/m/d h:i:s").'","btc":"'.$amount.'","state":"Paid","error":"1","errorTXT":"<div class="alert alert-dismissible alert-success"><strong>Payment Received! <i class="glyphicon glyphicon-ok"></i></strong><p>An amount of <b>$y$</b> has been added to your balance.</p></div>"}'; 
      }
      if ($row['state'] == "pending") {
        ...
        $sql2 = "INSERT INTO orders(amount,btcamount,username,lrpaidby,lrtrans,ip,state,date) VALUES('$y','$amount','$uid','$a','$a','n/a','Bitcoin',now())";
      ...
      echo '{"stop":"0","time":"'.date("Y/m/d h:i:s").'","btc":"'.$amount.'","state":"Not Paid","error":"0","errorTXT":"0"}';
    }
  }
?>
<?php
