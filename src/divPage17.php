...
$wherecond='';
$typesmtp=$_GET['smtp'];
$smtp = mysqli_real_escape_string($dbcon, $_GET['smtp']);
...
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`country`) FROM `dsmtps` WHERE `sold` = '0' $wherecond ORDER BY country ASC");
...
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`resseller`) FROM `dsmtps` WHERE `sold` = '0' $wherecond ORDER BY resseller ASC");
	while($row = mysqli_fetch_assoc($query)){
...
<thead>
 ...
 <?php
						   include("cr.php");
						   //echo "SELECT * FROM dsmtps WHERE sold='0' $wherecond ORDER BY RAND()";
     $q = mysqli_query($dbcon, "SELECT * FROM dsmtps WHERE sold='0' $wherecond ORDER BY RAND()")or die(mysql_error());
...
function buythistool(id){
  bootbox.confirm("Are you sure?", function(result) {
      ...
      $.ajax({
     method:"GET",
     url:"buytool.php?id="+id+"&t=dsmtps",
     dataType:"text",
     success:function(data){
...
//alert(id);  
	...
	$("#shop"+id).html('Sending...').show();	/*alert(xcheck); */		if(xcheck == '2'){				var getDataValue1 =  window.setInterval(function(){	$.ajax({	type: 		'GET',	url: 		'CheckSMTP'+id+'.html',	success:	function(data)	{		$("#shop"+id).html(data).show();		xcheck--;	}});	 window.clearInterval(getDataValue1);	},5000);			}else if(xcheck == '3'){		var getDataValue2 =  window.setInterval(function(){	$.ajax({	type: 		'GET',	url: 		'CheckSMTP'+id+'.html',	success:	function(data)	{		$("#shop"+id).html(data).show();		xcheck--;	}});	 window.clearInterval(getDataValue2);	},10000);	}else if(xcheck == '4'){		var getDataValue3 =  window.setInterval(function(){	$.ajax({	type: 		'GET',	url: 		'CheckSMTP'+id+'.html',	success:	function(data)	{		$("#shop"+id).html(data).show();		xcheck--;	}});	 window.clearInterval(getDataValue3);	},15000);	}else{
	$.ajax({
	type: 		'GET',
