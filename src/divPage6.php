...
$q = mysqli_query($dbcon, "SELECT * FROM leads WHERE sold='0' ORDER BY RAND()")or die(mysqli_error());
 while($row = mysqli_fetch_assoc($q)){
     ...
     echo "
      <tr>     
      <td id='leads_id'>".htmlspecialchars($row['id'])." </td>
      <td id='leads_country'><i class='flag-icon flag-icon-$countrycode'></i>&nbsp;".htmlspecialchars($row['country'])." </td>
      <td id='leads_source'>".htmlspecialchars($row['url'])." </td>
      <td id='leads_about'> ".htmlspecialchars($row['infos'])." </td> 
      <td> ".htmlspecialchars($row['number'])." </td>	<td><a href='#' class='btn btn-primary btn-xs getmodel' data-id='".$row['id']."'>Sample</a></td>
...
$(document).ready(function(){	
	...
	data:        {id:id}, 
	success:    function(data)    {   
	$("#modelbodysa").html(data).show();   
	}	
	});		
	$('#myModal_sample').modal('show');	
	})
	
	})
	</script>
	<div class="modal fade" id="myModal_sample" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">  
	<div class="modal-dialog" role="document">    
	<div class="modal-content">   
	<div class="modal-header">   
	<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>     
	...
	<div class="modal-body" id="modelbodysa">     
	</div>      
	<div class="modal-footer">   
	<button type="button" class="btn btn-primary" data-dismiss="modal">Close</button>   
	</div>   
