...
$query = mysqli_query($dbcon, "SELECT DISTINCT(`country`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY country ASC");
	while($row = mysqli_fetch_assoc($query)){
		
	$country .= '<option value="'.$row['country'].'">'.$row['country'].'</option>';
	}
	
	$query = mysqli_query($dbcon, "SELECT DISTINCT(`city`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY city ASC");
	while($row = mysqli_fetch_assoc($query)){
		
	$city .= '<option value="'.$row['city'].'">'.$row['city'].'</option>';
	}
	
		$query = mysqli_query($dbcon, "SELECT DISTINCT(`windows`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY windows ASC");
	while($row = mysqli_fetch_assoc($query)){
		
	$windows .= '<option value="'.$row['windows'].'">'.$row['windows'].'</option>';
	}
	
		$query = mysqli_query($dbcon, "SELECT DISTINCT(`access`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY access ASC");
	while($row = mysqli_fetch_assoc($query)){
		
	$access .= '<option value="'.$row['access'].'">'.$row['access'].'</option>';
	}
	
	$query = mysqli_query($dbcon, "SELECT DISTINCT(`hosting`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY hosting ASC");
	while($row = mysqli_fetch_assoc($query)){
		
	$hosting .= '<option value="'.$row['hosting'].'">'.$row['hosting'].'</option>';
	}
	
	$query = mysqli_query($dbcon, "SELECT DISTINCT(`resseller`) FROM `rdps` WHERE `sold` = '0' AND state='$cvl' ORDER BY resseller ASC");
	while($row = mysqli_fetch_assoc($query)){
	...
	print_r(json_encode(array("country"=>$country,"city"=>$city,"windows"=>$windows,"access"=>$access,"hosting"=>$hosting,"seller"=>$seller)));
}else{
	$country = '<option value="">All</option>';
	$city = '<option value="">All</option>';
	$windows = '<option value="">All</option>';
