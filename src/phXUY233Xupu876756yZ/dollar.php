...
<?php
  ...
  $qto = mysqli_query($dbcon, "SELECT SUM(price) as total FROM purchases WHERE date LIKE '$date%'");
  ...
  $qtr = mysqli_query($dbcon, "SELECT SUM(price) as total FROM purchases");
  $qtfr = mysqli_fetch_assoc($qtr);
  
   $rty = mysqli_query($dbcon,"select SUM(price) as total from  purchases WHERE MONTH(date) = MONTH(CURRENT_DATE())");
  $montot = mysqli_fetch_assoc($rty);
?>
...
<div class="form-group col-lg-3">
		<div class="teddy-text">
<center>	<b><font size="4" color="17C0FB">
<span class="glyphicon glyphicon-usd" style="font-size: 55px;"></span><br><h3><?php echo $montot['total']; ?>$</h3>
</font> </CENTER></b>
					</div>
			    <div class="teddy-follow">
