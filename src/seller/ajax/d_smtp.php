...
$qu = mysqli_query($dbcon, "SELECT * FROM dsmtps WHERE id='$id' AND resseller='$resseller'")or die("error");
...
global $price ;
 ...
 include "../../includes/config.php";
   $id = mysqli_real_escape_string($dbcon,$_GET['id']);
   $resseller = mysqli_real_escape_string($dbcon,$_SESSION['sname']);
   $query = mysqli_query($dbcon, "DELETE FROM smtps WHERE id='$id' AND resseller='$resseller'")or die();
   if($query){
  ...
  $q0 = mysqli_query($dbcon, "SELECT * FROM dsmtps WHERE id='$id' AND resseller='$resseller'")or die();
  if(mysqli_num_rows($q0) != 0 ){
    sqlquery('smtps',$price);
  }
