...
if($dataz != "") {
    ...
    flush();
		...
		$urlsdaatr = $url."/login";
		$encode = urlencode($urlsdaatr);
			$urltoapi = "https://vpox.to/apicheckcp.php?cp12=$encode&login=$login&pass=".rawurlencode($pass)."";
//$urltoapi = "http://46.226.108.66/apicheckcp.php?cp12=$cp1&login=$login&pass=".rawurlencode($pass)."";
...
$o = parse_url($url, PHP_URL_HOST);
		...
		$date = date('d/m/Y h:i:s a');
		$uid = mysqli_real_escape_string($dbcon, $_SESSION['sname']);
		mysqli_query($dbcon, "INSERT INTO cpanels(acctype,country,infos,url,price,sold,sto,dateofsold,resseller,date,reported,city,hosting,windows,ram,domain,price_show,dateof_sold,access)VALUES('cpanel','$country','$hostingg','$k','$price','0','0','$date','$uid','$date','0','0','0','0','0','0','$price','$date','0')") or die(mysqli_error($dbcon));
