  ...
  if(isset($_POST['start']) and $_POST['start'] == "work") {
    ...
    foreach ($site as $k) {
      ...
      if ($price == 0) {
        ...
        $match = preg_match('#Server IP Address :#', $hosting);
          ...
          if($data != 0) {
            ...
            flush();
	          ...
	          $o = parse_url($k, PHP_URL_HOST);
	          $check="SELECT * FROM mailers WHERE domain = '$o' and sold='0'";
            $rs = mysqli_query($dbcon,$check);
            $data = mysqli_num_rows($rs);
            ...
            if($data != "") {
              ...
              $sql = "INSERT INTO mailers(acctype,country,infos,url,price,resseller,sold,date,dateofsold,reported,sto,city,hosting,price_show,dateof_sold,access,windows,ram,domain)VALUES('mailer','$country','$hostingg','$k','$price','$uid','0','$date','0','0','0','0','$hostingg','$price','$date','0','0','0','0')";
              //echo $sql;
