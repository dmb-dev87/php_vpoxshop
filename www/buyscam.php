...
$scamname  = mysqli_real_escape_string($dbcon, $rows['scamname']);
...
if($balance >= $price){
  $newb = $balance - $price;
  $re = mysqli_query($dbcon, "SELECT sold FROM $tbl WHERE id='$uid'") or die("error");
  ...
  if($ree['sold'] == '0' OR $ree['sold'] == '1') {
    ...
    mysqli_query($dbcon, "INSERT INTO purchases(s_id,buyer,date,type,country,infos,url,login,pass,price,resseller,reported,reportid) VALUES ('$sid','$usrid','$date','$type','$fb','$scamname | $infos','$url','$login','$pa','$price','$resseller','','')") or die(mysqli_error($dbcon));
