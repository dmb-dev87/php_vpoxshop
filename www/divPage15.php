...
$q = mysqli_query($dbcon, "SELECT * FROM purchases WHERE buyer='$usrid' ORDER BY purchase_id DESC") or die(mysql_error());
...
<button onclick="openitem(<?php echo $idorder; ?>)" class="btn btn-primary btn-xs"> Open #<?php echo $idorder; ?></button>
	...
	$realtime=time();
	//echo $row['reported'].'--'.$realtime.' > '.$endTime; echo "<br>";
    if (($realtime > $endTime) && ($row['reported'] == "" || $row['reported']<1)) {
        echo 'Time expired';
    } else {
        if ($row['reported'] == "1") {
            $idreport = $row['id'];
