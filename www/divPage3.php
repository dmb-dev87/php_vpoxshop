...
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`country`) FROM `stufs` WHERE `sold` = '0' ORDER BY country ASC");
	while($row = mysqli_fetch_assoc($query)){
		$country = $row['country'];
		$query1 = mysqli_query($dbcon, "SELECT * FROM `stufs` WHERE  country='$country'");
...
<option value="">ALL</option>
<?php
$query = mysqli_query($dbcon, "SELECT DISTINCT(`windows`) FROM `stufs` WHERE `sold` = '0' ORDER BY windows ASC");
	while($row = mysqli_fetch_assoc($query)){
