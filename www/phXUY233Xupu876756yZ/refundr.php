...
<b>Refunded<br>Thank you for using Vpox.</b>
 </div>
  <div class="panel-footer"><div class="label label-danger">Admin</div> - '.date("d/m/Y h:i:s a").'</div>
  </div>
  ';
  $date = date("d/m/Y h:i:s a");
  $qq = mysqli_query($dbcon, "UPDATE reports SET memo = CONCAT(memo,'$msg'),status = ('0'),seen='1',lastreply='Admin',lastup='$date',state='accepted' WHERE id='$myid'")or die("mysql error");
