...
if(isset($_POST['log']) and  $_POST['log'] == "in"){
  ...
  $userpass = md5($pasa . $salt);
  $q = mysqli_query($dbcon, "SELECT * FROM users WHERE username='$user' AND password='$userpass'");
  echo mysqli_num_rows($q);
  $row = mysqli_fetch_assoc($q);
  if($user == $row['username'] And $userpass == $row['password']){
    $_SESSION['aname'] = $user;
    $_SESSION['apass'] = $pass;
...
<script src='https://www.google.com/recaptcha/api.js'></script>
  ...
  <title>VpoxShop - Admin Login</title>
  <!-- plugins:css -->
