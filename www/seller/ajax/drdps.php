...
$qu = mysqli_query($dbcon, "SELECT * FROM rdps WHERE id='$id' AND resseller='$resseller'")or die("error");
...
global $price ;
 ...
 function sqlquery($table,$pr){
   ...
   $id = mysqli_real_escape_string($dbcon, $_GET['id']);
   $resseller = mysqli_real_escape_string($dbcon, $_SESSION['sname']);
   $query = mysqli_query($dbcon, "DELETE FROM rdps WHERE id='$id' AND resseller='$resseller'")or die();
   if($query){
     echo "removed";
   }else{
  ...
  $q0 = mysqli_query($dbcon, "SELECT * FROM rdps WHERE id='$id' AND resseller='$resseller'")or die();
