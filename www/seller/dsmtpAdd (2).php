			 ...
			 if ($price == 0)
{
    ...
    flush();
		...
		$check="SELECT * FROM smtps WHERE url = '$link' and sold='1'";
...
if($data != "") {
 ...
 echo "[$i/$Total_lines]....".htmlspecialchars($host)." .. <img src='../loding2.gif' style='width:30px;position:relative;top:5px;' class='loader_data'/><b class='".$cid."' style='display:none'>Already Added !</b><br/>";
	...
	$check="SELECT * FROM smtps WHERE url = '$link' and sold='0'";
...
if($hostingg){
	$so = "INSERT INTO smtps(acctype,country,infos,price,url,sold,sto,dateofsold,resseller,reported,city,hosting,price_show,dateof_sold,access,windows,ram,domain,date)
	
	VALUES('smtp','$countryy','$hostingg','$price','$link','0','0','$date','$uid','0','0','$hostingg','$price','$date','0','0','0','0','$date')";
